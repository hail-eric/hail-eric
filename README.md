<h1 align="center">
  Hola 👋, Saya B Σ N (hail-eric)
</h1>

<h3 align="center">
  Seorang penjelajah <i>Dunia Digital</i> yang berfokus pada <i>Scripting</i>, kustomisasi jaringan, dan <i>Firmware Development</i>.
</h3>

<br>

<p align="center">
  Saya percaya bahwa teknologi harus bisa dioprek dan dioptimalkan. Saat ini, saya sedang mendalami dunia <i>coding</i> untuk memecahkan masalah-masalah praktis di sekitar saya.
</p>

---

<p align="center">
  <a href="https://github.com/hail-eric">
    <img src="https://github-readme-stats.vercel.app/api?username=hail-eric&show_icons=true&theme=tokyonight&count_private=true&include_all_commits=true" alt="Statistik GitHub" />
  </a>
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=20&pause=1000&color=58A6FF&center=true&width=435&lines=There+are+no+coincidences;+Only+the+illusion+of+coincidences+👁️" alt="Animasi Teks" />
</p>

---

### 🔭 Fokus Saya Saat Ini:

Di sinilah sebagian besar waktu saya dihabiskan:

* 📱 **Termux & Scripting:** Membuat skrip automasi (termasuk untuk <i>mining</i>) yang berjalan efisien di perangkat *low-power*.
* 📡 **OpenWrt & Networking:** Meracik konfigurasi OpenClash yang optimal dan stabil untuk manajemen jaringan yang presisi.
* 🔧 **Firmware Engineering:** Dalam proses belajar membangun *custom firmware* OpenWrt dari *ImmortalWrt* untuk kontrol penuh atas perangkat.

---

### 💻 Tools & Teknologi Pilihan Saya:

Ini adalah beberapa hal yang sedang saya gunakan dan pelajari:

<p align="center">
  <img src="https://skillicons.dev/icons?i=linux,bash,python,go,docker,git,github,vscode" alt="My Tech Stack"/>
</p>

---

### 📊 Statistik Aktivitas Saya di GitHub:

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=hail-eric&show_icons=true&theme=solarized_dark&hide_border=true" alt="B Σ N's GitHub Stats" />
  
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=hail-eric&layout=compact&theme=solarized_dark&hide_border=true" alt="B Σ N's Top Languages" />
</p>

---

### 📬 Hubungi Saya:

Mari berdiskusi atau berkolaborasi!

<p align="center">
  <a href="https://t.me/hail_ct0S" target="_blank">
    <img src="https://img.shields.io/badge/Telegram-@hail_ct0S-26A5E4?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram: @hail_ct0S">
  </a>
</p>

<br>

<h3 align="center">
  There are no <i>coincidences</i>, only the illusion of <i>coincidences</i> 👁️
</h3>
